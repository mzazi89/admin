...
export default function DocsApp({ endpoints }) {
  ...
  return (
    <div className="container-site py-12" style={{ minHeight: '70vh' }}>
      {/* Header */}
      <div className="mb-6">
        <a href="/api" className="text-xs font-semibold" style={{ color: 'var(--muted)', textDecoration: 'none' }}>← Back to API</a>
        ...
        {[['endpoints', 'Endpoints'], ['guides', 'Guides']].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)}
            ...
            style={{
              ...
              color: tab === id ? 'var(--blue-bright)' : 'var(--dim)',
              border: `1px solid ${tab === id ? 'rgba(37,99,235,0.4)' : 'var(--blue-deep)'}`,
              cursor: 'pointer',
            }}>
            {label}
          </button>
        ...
        <div className="max-w-2xl space-y-6">
          {GUIDES.map(g => (
            <section key={g.id} className="card p-6">
              <h2 className="font-bold mb-2" style={{ color: 'var(--ink)' }}>{g.title}</h2>
              <p className="text-sm leading-relaxed mb-3" style={{ color: 'var(--dim)' }}>{g.body}</p>
              {g.code && <CodeBlock label={g.title} code={g.code} />}
            </section>
          ))}
          <section className="card p-6">
            <h2 className="font-bold mb-2" style={{ color: 'var(--ink)' }}>Support</h2>
            <p className="text-sm" style={{ color: 'var(--dim)' }}>
              Need help? Check the <a href="/api/status" style={{ color: 'var(--blue-bright)' }}>status page</a>, the
              {' '}<a href="/api/explorer" style={{ color: 'var(--blue-bright)' }}>explorer</a>, or the
              {' '}<a href="/contact" style={{ color: 'var(--blue-bright)' }}>contact page</a>.
            </p>
          </section>
        </div>
      ) : (
          ...
          <aside className="lg:col-span-1">
            <div className="card p-4 sticky top-24">
              ...
              <button onClick={() => { setActiveCategory('ALL'); setSearch(''); }}
                ...
                style={{ backgroundColor: activeCategory === 'ALL' ? 'rgba(37,99,235,0.12)' : 'transparent', color: activeCategory === 'ALL' ? 'var(--blue-bright)' : 'var(--dim)' }}>
                All categories <span className="text-xs" style={{ color: 'var(--muted)' }}>({activeCount} live)</span>
              </button>
              {categories.map(c => (
                <button key={c.name} onClick={() => setActiveCategory(activeCategory === c.name ? 'ALL' : c.name)}
                  className="w-full text-left px-3 py-2 rounded-lg text-sm"
                  style={{ backgroundColor: activeCategory === c.name ? 'rgba(37,99,235,0.12)' : 'transparent', color: activeCategory === c.name ? 'var(--blue-bright)' : 'var(--dim)' }}>
                  {CATEGORY_ICONS[c.name] || '📦'} {CATEGORY_LABELS[c.name] || c.name}
                  <span className="text-xs ml-1" style={{ color: 'var(--muted)' }}>{c.active}/{c.total}</span>
                </button>
              ))}
            </div>
          </aside>
                    ...
                    return (
                      <div key={e.path} className="card overflow-hidden" style={{ opacity: e.is_active ? 1 : 0.55 }}>
                        <button onClick={() => setExpanded(isOpen ? null : e.path)}
                          ...
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded" style={{ backgroundColor: e.method === 'GET' ? 'var(--blue-deep)' : 'var(--brand-deep)', color: e.method === 'GET' ? 'var(--blue-soft)' : 'var(--brand-soft)' }}>
                            {e.method}
                          </span>
                          <code className="text-xs font-mono flex-1" style={{ color: 'var(--ink-2)' }}>{e.path}</code>
                          <span className="text-xs font-semibold hidden sm:block" style={{ color: 'var(--dim)' }}>{e.name}</span>
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded" style={{
                            backgroundColor: e.is_active ? 'rgba(74,222,128,0.1)' : 'rgba(100,116,139,0.1)',
                            color: e.is_active ? 'var(--good)' : 'var(--muted)',
                          }}>
                        ...
                        {isOpen && (
                          <div className="px-5 pb-5 space-y-4" style={{ borderTop: '1px solid var(--bg)' }}>
                            <p className="text-sm pt-4" style={{ color: 'var(--dim)' }}>
                              {e.description || e.name} · Provider: <code className="text-xs" style={{ color: 'var(--blue-soft)' }}>{e.provider || '—'}</code>
                            </p>
                                ...
                                <table className="w-full text-sm">
                                  <thead>
                                    <tr style={{ borderBottom: '1px solid var(--blue-deep)' }}>
                                      <th className="text-left px-3 py-2 text-xs font-bold uppercase" style={{ color: 'var(--muted)' }}>Parameter</th>
                                      <th className="text-left px-3 py-2 text-xs font-bold uppercase" style={{ color: 'var(--muted)' }}>Required</th>
                                      <th className="text-left px-3 py-2 text-xs font-bold uppercase" style={{ color: 'var(--muted)' }}>Example</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    {[...req.map(n => ({ n, req: true })), ...opt.map(n => ({ n, req: false }))].map(({ n, req: isReq }) => {
                                      ...
                                      return (
                                        <tr key={n} style={{ borderBottom: '1px solid var(--bg)' }}>
                                          <td className="px-3 py-2 font-mono text-xs" style={{ color: 'var(--ink-2)' }}>{n}</td>
                                          <td className="px-3 py-2 text-xs" style={{ color: isReq ? 'var(--bad)' : 'var(--muted)' }}>{isReq ? 'Yes' : 'No'}</td>
                                          <td className="px-3 py-2 font-mono text-xs break-all" style={{ color: 'var(--muted)' }}>{example || '—'}</td>
                                        </tr>
                                      );
                                    })}
                                  </tbody>
                            ...
                            <div>
                              <p className="text-[10px] font-bold uppercase tracking-wide mb-1.5" style={{ color: 'var(--muted)' }}>Example request</p>
                              <CodeBlock label="curl" code={`curl "${BASE}${e.path}?${[...req, ...opt].map(n => `${n}=YOUR_VALUE`).join('&')}${(req.length || opt.length) ? '&' : ''}apikey=YOUR_API_KEY"`} />
                            </div>

                            {e.is_active ? (
                              <div>
                                <p className="text-[10px] font-bold uppercase tracking-wide mb-1.5" style={{ color: 'var(--muted)' }}>Live tester</p>
