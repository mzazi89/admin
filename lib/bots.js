...
export async function resolveBotForNumber(number, requested) {
  ...
  // With one bot there is nothing to disambiguate, so the row stays untargeted —
  // exactly the row this panel wrote before bots were selectable. Stamping the
  // fallback id "main" would look equivalent today and then strand the request
  // if bot_profiles is switched on before the bot claims it.
