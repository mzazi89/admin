...
export async function initializeDatabase() {
  try {
    ...
    // ── Bot profiles on the command registry ────────────────────────────────
    // The admin boots the database too, so it must apply the SAME schema as the
    // website. Without this, a deployment where only the admin runs would never
    // get the profile column or the scoped uniqueness below — and its own seed
    // (which still says ON CONFLICT (name)) would start failing.
    ...
    // Uniqueness must be per bot, not global: QUARTZ XD and MZAZI XMD both want
    // a `menu`, and a global name constraint makes that impossible — the second
    // bot's seed then fails on every name the first one already has.
    ...
    // bot_control.bot_id is how a control row (including `sync`) is addressed to
    // one specific bot. Empty/NULL means "any bot", which is the old behaviour.
    ...
    // settings — key/value config shared with the bot (Paystack, Pterodactyl…)
    await sql`
    ...
    // ── VPS store (packages + credential pool + orders) ──
    await sql`
    ...
    // ── WhatsApp panel resellers (reseller passwords + created panels) ──
    await sql`
    ...
    // The Pterodactyl servers the admin site can manage. This replaces the single
    // pterodactyl_url / pterodactyl_api_key pair in `settings`, which is still read
    // as a fallback when this table is empty — so a deployment configured before
    // this table existed keeps working, and gains a row the first time the admin
    // opens the page (see seedFromLegacySettings in lib/pterodactylPanels.js).
    await sql`
    ...
    // clear the old one first. Without this, two defaults can exist after a
    // half-failed update and which panel gets used becomes arbitrary.
    await sql`
      CREATE UNIQUE INDEX IF NOT EXISTS pterodactyl_panels_one_default
        ON pterodactyl_panels (is_default) WHERE is_default;
    `;
    ...
    if (fs.existsSync(botCommandsPath)) {
      ...
      if (parseInt(botCount[0].cnt) < seedList.length) {
        ...
        for (const cmd of seedList) {
          try {
            // `profile` is carried through so a registry describing more than one
            // bot seeds each of them. Dropping it here would file every entry
            // under the primary bot, the second bot would be empty, and the
            // ON CONFLICT target (profile, name) would then silently discard the
            // rows that were meant to be distinct.
            const profile = typeof cmd.profile === 'string' && /^[A-Za-z0-9_-]{0,64}$/.test(cmd.profile) ? cmd.profile : '';
            const r = await sql`
              ...
              VALUES (${cmd.name}, ${JSON.stringify(cmd.aliases || [])}::jsonb, ${cmd.description || ''}, ${cmd.category || 'General'},
                      ...
                      ${cmd.enabled !== false}, ${cmd.code || ''}, ${profile})
